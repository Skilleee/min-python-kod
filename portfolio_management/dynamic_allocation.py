...
def dynamic_allocation(portfolio, market_trends):
    ...
    """
    try:
        portfolio["adjustment"] = portfolio["sector"].apply(lambda x: 0.05 if x in market_trends else -0.05)
        portfolio["new_allocation"] = np.clip(portfolio["allocation"] + portfolio["adjustment"], 0, 1)
