...
def track_signal_performance(signals, actual_prices):
    ...
    """
    try:
        signals["actual_return"] = (actual_prices["close"] - actual_prices["close"].shift(1)) / actual_prices["close"].shift(1)
        signals["success"] = (signals["signal"] == "BUY") & (signals["actual_return"] > 0) | (signals["signal"] == "SELL") & (signals["actual_return"] < 0)
...
if __name__ == "__main__":
    ...
    actual_prices_df = pd.DataFrame({
        "date": pd.date_range(start="2023-01-01", periods=300),
        "close": (100 + pd.Series(range(300)) + pd.Series(range(300)).apply(lambda x: x % 5 - 2)).cumsum()
    })
