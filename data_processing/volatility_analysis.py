...
def calculate_daily_volatility(price_series):
    ...
    try:
        ...
        logging.info(f"[{datetime.now()}] ✅ Daglig volatilitet beräknad: {daily_volatility:.6f}")
        return daily_volatility
    except Exception as e:
        logging.error(f"[{datetime.now()}] ❌ Fel vid beräkning av daglig volatilitet: {str(e)}")
        return None
...
def calculate_annual_volatility(price_series):
    ...
    try:
        ...
        logging.info(f"[{datetime.now()}] ✅ Årlig volatilitet beräknad: {annual_volatility:.6f}")
        return annual_volatility
    except Exception as e:
        logging.error(f"[{datetime.now()}] ❌ Fel vid beräkning av årlig volatilitet: {str(e)}")
        return None
...
def analyze_historical_volatility(price_series, window=30):
    ...
    try:
        ...
        logging.info(f"[{datetime.now()}] ✅ Historisk volatilitet analyserad över {window} dagar.")
        return rolling_volatility
    except Exception as e:
        logging.error(f"[{datetime.now()}] ❌ Fel vid analys av historisk volatilitet: {str(e)}")
        return None
