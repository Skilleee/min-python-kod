...
def analyze_macro_factors(macro_data):
    ...
    try:
        ...
        ]  # Högre räntor påverkar negativt
        inflation_impact = (
            -macro_data["inflation_rate"] * 0.5
        )  # Hög inflation påverkar marknaden negativt
        gdp_growth_impact = (
            macro_data["gdp_growth"] * 0.7
        )  # Högre BNP-tillväxt är positivt
...
def adjust_decision_based_on_macro(macro_data, trade_log, threshold=0.5):
    ...
    try:
        ...
        logging.error(
            f"❌ Fel vid justering av beslut baserat på makrofaktorer: {str(e)}"
        )
        return None
