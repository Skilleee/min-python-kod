...
def calculate_optimal_allocation(trade_log, risk_metrics, max_allocation=0.3, min_allocation=0.05):
    """
    ...
    try:
        ...
        normalized_return = (trade_log["return"] - trade_log["return"].min()) / (trade_log["return"].max() - trade_log["return"].min())
        normalized_risk = (risk_metrics["volatility"] - risk_metrics["volatility"].min()) / (risk_metrics["volatility"].max() - risk_metrics["volatility"].min())
...
if __name__ == "__main__":
    ...
    risk_metrics = pd.DataFrame({
        "volatility": [0.02, 0.05, 0.03, 0.04, 0.06]
    })
