...
def evaluate_trade_performance(trade_log):
    ...
    try:
        ...
        logging.info(
            f"✅ Handelsutvärdering klar: Win rate: {win_rate:.2%}, Genomsnittlig avkastning: {avg_return:.2%}"
        )
...
def refine_trading_strategy(trade_log, threshold=0.02):
    ...
    try:
        ...
        logging.info(
            f"✅ Strategianalys klar: {len(good_trades)} bra trades, {len(bad_trades)} dåliga trades"
        )
