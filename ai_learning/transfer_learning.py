...
def fine_tune_model(base_model, new_data, output_classes=2, learning_rate=0.001):
    ...
    """
    try:
        feature_columns = [col for col in new_data.columns if col not in ["symbol", "date", "target"]]
        ...
        fine_tuned_model.compile(optimizer=keras.optimizers.Adam(learning_rate=learning_rate),
                                 loss="sparse_categorical_crossentropy",
                                 metrics=["accuracy"])
        
        fine_tuned_model.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=10, batch_size=32)
