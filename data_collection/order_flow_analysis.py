...
def fetch_order_flow(symbol):
    ...
    try:
        ...
        logging.info(f"[{datetime.now()}] ✅ Orderflöde för {symbol}: Buy Orders: {buy_orders}, Sell Orders: {sell_orders}, Net: {net_flow}")
        return {"buy_orders": buy_orders, "sell_orders": sell_orders, "net_flow": net_flow}
...
def analyze_order_flow_sentiment(symbol):
    ...
    if flows:
        ...
        logging.info(f"[{datetime.now()}] 📊 Orderflödessentiment för {symbol}: {sentiment}")
        return sentiment
    else:
        return "unknown"
