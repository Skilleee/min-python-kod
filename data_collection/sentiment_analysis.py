...
def fetch_tweets(keyword, count=10):
    ...
    """
    try:
        headers = {"Authorization": "Bearer YOUR_TWITTER_BEARER_TOKEN"}  # Ersätt med giltig API-nyckel
        ...
        logging.info(f"[{datetime.now()}] ✅ Hämtade {len(tweets)} tweets för {keyword}")
        return tweets
    except Exception as e:
        logging.error(f"[{datetime.now()}] ❌ Fel vid hämtning av tweets för {keyword}: {str(e)}")
        return []
...
def fetch_reddit_posts(subreddit="stocks", count=10):
    ...
    try:
        ...
        data = response.json()
        posts = [post["data"]["title"] for post in data.get("data", {}).get("children", [])[:count]]
        logging.info(f"[{datetime.now()}] ✅ Hämtade {len(posts)} Reddit-inlägg från {subreddit}")
        return posts
    except Exception as e:
        logging.error(f"[{datetime.now()}] ❌ Fel vid hämtning av Reddit-inlägg: {str(e)}")
        return []
...
def analyze_sentiment(texts):
    ...
    logging.info(f"[{datetime.now()}] 📊 Sentimentanalys: {sentiment} (Polarity: {total_polarity})")
    return sentiment
