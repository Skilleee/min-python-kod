...
def fetch_avanza_stock(symbol):
    ...
    try:
        ...
        logging.error(f"[{datetime.now()}] ❌ Fel vid hämtning av Avanza-data för {symbol}: {str(e)}")
        return None
...
def fetch_binance_price(symbol):
    ...
    try:
        ...
        logging.info(f"[{datetime.now()}] ✅ Hämtade Binance-data för {symbol}: {price} USD")
        return price
    except Exception as e:
        logging.error(f"[{datetime.now()}] ❌ Fel vid hämtning av Binance-data för {symbol}: {str(e)}")
        return None
...
def fetch_alpaca_stock(symbol):
    ...
    try:
        ...
        logging.error(f"[{datetime.now()}] ❌ Fel vid hämtning av Alpaca-data för {symbol}: {str(e)}")
        return None
